    ...
    $scope.mcdb.query("materialscommons-app","tags_by_count", {group_level:1}).success(function(){
        if ($scope.mcdb.rows.length > 0) {
            ...
            angular.forEach($scope.mcdb.rows, function(row){
                $scope.word_list.push({text: row.key[0], weight: row.value, link: "http://"+host_name+":5984/materialscommons/_design/materialscommons-app/index.html#/tags/data/bytag/"+row.key[0]+"/esitzmann"});
