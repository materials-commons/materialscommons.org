...
const fileUtils = require('../util/create-file-utils');
...
async function getFileDatasets(fileId) {
    return await r.table("dataset2datafile").getAll(fileId, {index: 'datafile_id'})
        .eqJoin("dataset_id", r.table("datasets")).zip();
}

async function getFileSimple(fileId) {
    return await r.table('datafiles').get(fileId);
}
...
async function getFileProjects(fileId) {
    return await r.table("datadir2datafile").getAll(fileId, {index: 'datafile_id'})
