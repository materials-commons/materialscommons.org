...
async function deleteExperimentFull(projectId, experimentId, options) {
    ...
    let fileLinkIds = await deleteExperimentFileLinks(experimentId, dryRun);
    ...
    overallResults['notes'] = await deleteNotes(allPosibleItems, dryRun);
    overallResults['reviews'] = await deleteReviews(allPosibleItems, dryRun);
...
async function deleteExperimentFileLinks(experimentId, dryRun) {
    ...
    if (!dryRun) {
        let delete_msg = await r.table('experiment2datafile')
            .getAll(experimentId, {index: 'experiment_id'}).delete();
    }
