...
curl --verbose -XPOST -H"Content-Type: application/json" -d'{"template_type": "process","template_name": "Run SEM1","owner": "mcfada@umich.edu","template_description": "Collect data using an SEM.","required_output_conditions": ["material_properties"], "required_conditions": ["sem_equipment_properties", "material_properties"]}' 'http://localhost:'${PORT}'/templates/new?apikey='${KEY}

curl --verbose -XPOST -H"Content-Type: application/json" -d'{"template_type": "process","template_name": "APT","owner": "mcfada@umich.edu","template_description": "Collect data using an APT.","required_conditions": ["material_properties"], "required_output_conditions": []}' 'http://localhost:'${PORT}'/templates/new?apikey='${KEY}
