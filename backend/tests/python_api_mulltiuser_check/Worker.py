...
class Worker:
    ...
    def do_some_api_work(self):
        ...
        self.log.info("project owner = {}, worker apikey = {}".format(project.owner, self.apikey))
        ...
        for process in process_list:
            if create_process.id == process.id:
                found1 = process
            if ht_process.id == process.id:
                found2 = process
        if found1:
            self.log.info("found process {}".format(found1.name))
        else:
            self.log.info("did not find 'create samples' process")
        if found2:
            self.log.info("found process {}".format(found2.name))
        else:
            self.log.info("did not find 'heat treatment' process")
