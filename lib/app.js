exports.views = {
    ...
    data_with_datagroups: {
        map: function(doc) {
            if (doc.type == "data") {
                emit([doc._id, doc.owner, 0], doc);
            }
            else if (doc.type == "datagroup") {
                doc.data.forEach(function(dgdoc) {
                    doc.users.forEach(function(user) {
                        emit([dgdoc, user, 1], doc);
                    });
                });
            }
        }
    ...
    datagroup_by_user: {
        map: function (doc) {
            if (doc.type == "datagroup") {
                emit([doc.users[0], doc.access], doc);
            }
        }
    },
...
//    },
    ...
    datagroup_by_usergroups: {
        map: function (doc) {
            if (doc.type == "datagroup") {
                if(doc.users[0]=="virm" || doc.users[0] == "miaojias"){
                    emit("allisonlab", doc);
                }
                else if(doc.users[0]=="mcfada" || doc.users[0] == "pengwchu" || dco.users[0] =="esitzmann"){
                    emit("marquislab", doc);
                }
